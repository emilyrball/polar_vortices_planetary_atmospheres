...
def calculate_eddy_enstrophy(PV, level, Ls, MY):
    #PV = fn of isentropic level, time, lat, lon
    if level == 'mean':
        q = PV.mean(dim='ilev')
    else:
        q = PV.sel(ilev=PV.ilev[level])

    northern = np.zeros_like(q.lat)
    for i in range(len(q.lat)):
        if q.lat[i] > 0:
            northern[i] = 1
    
    q = q.sel(lat = q.lat[northern == 1.0])

    #q = q.sel(lat=q.lat[2:6])
    qbar = q.mean(dim='lon')
    qbar1 = qbar.expand_dims({'lon':q.lon})
    ...
    newyear=np.zeros_like(MY)
    for i in range(len(MY)-1):
        if MY[i+1] > MY[i]:
            newyear[i+1]=newyear[i]+1
        else:
            newyear[i+1]=newyear[i]
    ls0=Ls.sel(time=PV.time[newyear==0])
    ls1=Ls.sel(time=PV.time[newyear==1])
    ls2=Ls.sel(time=PV.time[newyear==2])
    ls3=Ls.sel(time=PV.time[newyear==3])

    fig = plt.figure(figsize=(15,5))
    ax = plt.axes()
    
    ls0_tick = list(np.linspace(ls0[0], ls0[-1],len(ls0.time)))
    ls1_tick = list(np.linspace(ls1[0], ls1[-1],len(ls1.time)))
    ls2_tick = list(np.linspace(ls2[0], ls2[-1],len(ls2.time)))
    ls3_tick = list(np.linspace(ls3[0], ls3[-1],len(ls3.time)))

    ticks = ls0_tick+ls1_tick+ls2_tick+ls3_tick

    ticks = ticks[0:2000]
    ticks = ticks[slice(None,None,500)]
    ticks = [10*int(ticks[i]/10) for i in range(len(ticks))]
    ax.plot(PV.time[0:2000], Z[0:2000])
    plt.xticks(PV.time[0:2000][slice(None,None,500)], ticks, rotation='vertical')
    plt.xlabel('solar longitude')
    plt.ylabel('eddy enstrophy')
    plt.title('Eddy enstrophy evolution on the 250K surface')
    #ls = np.linspace(248,286,len(Z.time))
    plt.tight_layout()

    plt.savefig('eddyenstrophy_Ls'+str(ticks[0])+'-'+str(ticks[-1])+'.pdf')

    print(np.min(Z[1750:2000].time.values))
