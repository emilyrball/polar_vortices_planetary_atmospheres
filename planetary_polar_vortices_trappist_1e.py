...
if __name__ == "__main__":
    ...
    d = d.where(d.latitude < 80, drop=True)
    ...
    t = d.air_temperature.sel(air_pressure=plev, method='nearest').mean(dim='time')
    t = t.where(t.latitude>=0,drop=True)
    ...
    cb = fig.colorbar(cm.ScalarMappable(norm=norm,cmap=cmap),
                 ...
                 shrink=0.9, pad=.03,ticks=boundaries[slice(1,None,2)])
    ...
    u = d.x_wind.sel(air_pressure=plev, method='nearest').mean(dim='time')
    u = u.where(u.latitude>=0,drop=True)


    u = u.sel(latitude = list(np.arange(10,75,10)),method='nearest')
    u = u.sel(longitude = list(np.arange(-180,180,30)),method='nearest')

    v = d.y_wind.sel(air_pressure=plev, method='nearest').mean(dim='time')
    v = v.where(v.latitude>=0,drop=True)
